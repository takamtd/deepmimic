...
from pybullet_envs.deep_mimic.env import atlas_stable_pd
...
class PyBulletDeepMimicEnv(Env):
  ...
  def reset(self):
    ...
    if not self._isInitialized:
      ...
      useFixedBase = False
      # self._humanoid = humanoid_stable_pd.AtlasStablePD(self._pybullet_client, self._mocapData,
      #                                                      timeStep, useFixedBase, self._arg_parser)
      self._humanoid = atlas_stable_pd.AtlasStablePD(self._pybullet_client, self._mocapData,timeStep, useFixedBase, self._arg_parser)
  ...
  def get_action_size(self, agent_id):
    # ctrl_size = 43  #numDof
    root_size = 7
    return self._humanoid._totalDofs - root_size
  ...
  def build_action_offset(self, agent_id):
    out_offset = [0.01] * self.get_action_size(agent_id)
    # out_offset = [
  ...
  #                                     initBase=True)
  #       self._humanoid.initializePoseAtlas(self._humanoid._poseInterpolator,
  #                                     self._humanoid._sim_model,
  #                                     initBase=True)
  ...
  def update(self, timeStep):
    ...
    for i in range(1):
      ...
      if self.desiredPose:
        ...
        # ]
        # linkStatesKin = self._humanoid._pybullet_client.getLinkStates(self._humanoid._kin_model, kin_jointIndices, computeForwardKinematics=True, computeLinkVelocity=True)
